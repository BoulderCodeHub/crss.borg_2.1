...
adjustDates <- function(inputFilePath, outputFilePath) {
 ...
 # df <- within(df, end_month[start_month == i] <- 9)
  ...
  #df <- within(df, end_year[start_month == i] <- (2060))
  #df <- within(df, num_timesteps[start_month == i] <- (12*(2060 - start_year+1)))
 ...
 # change end date and run length
  for (i in c(1:12)) {
    ...
    df <- within(df, num_timesteps[start_month == i] <- ((2060-start_year+1)*12))
    df <- within(df, end_year[start_month == i] <- (2060))
  }
